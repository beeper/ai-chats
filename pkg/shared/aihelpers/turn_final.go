...
func (t *Turn) sendFinalEdit(ctx context.Context, finishReason string) {
	...
	if err := t.ensureSenderJoined(ctx, sender, bridgev2.RemoteEventMessage); err != nil && t.conv.login != nil {
		t.conv.login.Log.Warn().Err(err).Str("component", "ai_turn").Msg("Failed to join sender before final turn edit")
	}
	if err := SendEditViaPortal(
		...
		t.conv.login.Log.Warn().Err(err).Str("component", "ai_turn").Msg("Failed to send final turn edit")
	}
}
