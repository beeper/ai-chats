...
type testAIHelperClient struct {
	baseTestClient
	updated int
}
...
func TestNewConnectorBaseUsesHooksAndCustomClients(t *testing.T) {
	...
	if _, ok := login.Client.(*testAIHelperClient); !ok {
		t.Fatalf("expected testAIHelperClient, got %T", login.Client)
	}
...
func TestNewConnectorBaseUsesCustomLoadLoginAndLoginFlows(t *testing.T) {
	...
	cfg := &Config[*struct{}, *struct{}]{
		...
		LoadLogin: func(_ context.Context, login *bridgev2.UserLogin) error {
			loadCalled++
			login.Client = &testAIHelperClient{}
			return nil
		},
	...
	if _, ok := login.Client.(*testAIHelperClient); !ok {
		t.Fatalf("expected custom load login to set testAIHelperClient, got %T", login.Client)
	}
...
var _ bridgev2.NetworkAPI = (*testAIHelperClient)(nil)
