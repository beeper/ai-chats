...
func TestBuildFinalEditUIMessage_IncludesSourceAndFileParts(t *testing.T) {
	...
	ui := aihelpers.BuildCompactFinalUIMessage(oc.buildStreamUIMessage(state, modelModeTestMeta("openai/gpt-4.1"), nil))
	if ui == nil {
...
func TestBuildFinalEditPayloadKeepsOnlyEditMetadataTopLevel(t *testing.T) {
	...
	payload := aihelpers.BuildFinalEditPayload(event.MessageEventContent{
	...
	if _, ok := extra["m.relates_to"]; ok {
		t.Fatalf("expected AIHelper to inject m.relates_to, got %#v", extra["m.relates_to"])
	}
