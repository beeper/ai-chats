...
func (s *streamingState) writer() *aihelpers.Writer {
	if s == nil || s.turn == nil {
		return nil
	}
...
func (s *streamingState) nextMessageTiming() aihelpers.EventTiming {
	if s == nil {
		return aihelpers.ResolveEventTiming(time.Time{}, 0)
	}
	ts := time.UnixMilli(s.startedAtMs)
	if s.startedAtMs <= 0 {
		ts = time.Now()
	}
	timing := aihelpers.NextEventTiming(s.lastStreamOrder, ts)
