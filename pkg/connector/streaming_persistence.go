...
func (oc *AIClient) buildStreamingMessageMetadata(state *streamingState, meta *PortalMetadata, turnData aihelpers.TurnData) *MessageMetadata {
	if state == nil {
		return nil
	}
	turn := state.turn
	...
	bundle := aihelpers.BuildAssistantMetadataBundle(aihelpers.AssistantMetadataBundleParams{
		TurnData:           turnData,
		ToolType:           "ai",
...
func (oc *AIClient) saveAssistantMessage(
	...
	var turnData aihelpers.TurnData
	if state.turn != nil {
	...
	if messageID == "" && initialEventID != "" {
		messageID = aihelpers.MatrixMessageID(initialEventID)
	}
