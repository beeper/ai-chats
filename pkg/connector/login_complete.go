...
func (ol *OpenAILogin) completeLogin(ctx context.Context, input loginCompletionInput) (*bridgev2.LoginStep, error) {
	...
	if override != nil {
		...
		if !strings.EqualFold(normalizeProvider(overrideMeta.Provider), provider) {
			return nil, aihelpers.NewLoginRespError(http.StatusBadRequest, fmt.Sprintf("Can't relogin %s account with %s credentials.", overrideMeta.Provider, provider), "AI", "PROVIDER_MISMATCH")
		}
	}
	...
	if override != nil {
		...
		if err != nil {
			return nil, aihelpers.WrapLoginRespError(fmt.Errorf("failed to clone relogin metadata: %w", err), http.StatusInternalServerError, "AI", "CLONE_RELOGIN_METADATA_FAILED")
		}
		cfg, err = loadAILoginConfig(ctx, override)
		if err != nil {
			return nil, aihelpers.WrapLoginRespError(fmt.Errorf("failed to load relogin config: %w", err), http.StatusInternalServerError, "AI", "LOAD_RELOGIN_CONFIG_FAILED")
		}
	}
	if meta == nil {
		meta = &UserLoginMetadata{}
	...
	login, step, err := aihelpers.PersistAndCompleteLoginWithOptions(
		ctx,
		context.Background(),
		ol.User,
		&database.UserLogin{
	...
	if err != nil {
		...
		return nil, aihelpers.WrapLoginRespError(fmt.Errorf("failed to complete login: %w", err), http.StatusInternalServerError, "AI", code)
	}
	return step, nil
}
