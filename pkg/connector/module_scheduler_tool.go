...
func executeCron(ctx context.Context, args map[string]any) (string, error) {
	...
	case "add":
		...
		job, err := cronModule.Add(jobInput)
		if err != nil {
			return agenttools.JSONResult(map[string]any{
				"status": "error",
				"error":  err.Error(),
	...
	case "update":
		...
		job, err := cronModule.Update(jobID, patch)
		if err != nil {
			return agenttools.JSONResult(map[string]any{
				"status": "error",
				"error":  err.Error(),
	...
	case "run":
		...
		ran, reason, err := cronModule.Run(jobID, "")
		if err != nil {
			return agenttools.JSONResult(map[string]any{
				"status": "error",
				"error":  err.Error(),
	...
	case "runs":
		...
		runs, err := cronModule.Runs(jobID, limit)
		if err != nil {
			return agenttools.JSONResult(map[string]any{
				"status": "error",
				"error":  err.Error(),
	...
	case "wake":
		...
		_, err := cronModule.Wake(mode, text)
		if err != nil {
			return agenttools.JSONResult(map[string]any{
				"status": "error",
				"error":  err.Error(),
