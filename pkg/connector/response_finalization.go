...
func (oc *AIClient) sendContinuationMessage(ctx context.Context, portal *bridgev2.Portal, body string, replyTarget ReplyTarget, timing aihelpers.EventTiming) {
	if portal == nil || portal.MXID == "" {
		return
	}
	sender := oc.senderForPortal(ctx, portal)
	rendered := format.RenderMarkdown(body, true, true)
	msgID := aihelpers.NewMessageID("ai")
	...
	if _, _, err := aihelpers.SendViaPortal(aihelpers.SendViaPortalParams{
		Login:       oc.UserLogin,
		Portal:      portal,
		Sender:      sender,
...
func (oc *AIClient) sendFinalAssistantTurnContent(ctx context.Context, portal *bridgev2.Portal, state *streamingState, meta *PortalMetadata, markdown string, rendered event.MessageEventContent, replyTarget ReplyTarget, mode string) {
	...
	uiMessage := aihelpers.BuildCompactFinalUIMessage(oc.buildStreamUIMessage(state, meta, linkPreviews))
	...
	if state != nil && state.turn != nil {
		...
		state.turn.SetFinalEditPayload(aihelpers.BuildFinalEditPayload(
			rendered,
			uiMessage,
