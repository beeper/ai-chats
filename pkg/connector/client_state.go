...
func (oc *AIClient) loggerForContext(ctx context.Context) *zerolog.Logger {
	return aihelpers.LoggerFromContext(ctx, &oc.log)
}
	...
	for _, entry := range entries {
		...
		if userMessage == nil {
			...
			userMessage = &database.Message{
				ID:       aihelpers.MatrixMessageID(entry.Event.ID),
				MXID:     entry.Event.ID,
				Room:     entry.Portal.PortalKey,
				SenderID: humanUserID(oc.UserLogin.ID),
				Metadata: &MessageMetadata{
					BaseMessageMetadata: aihelpers.BaseMessageMetadata{Role: "user", Body: entryBody},
				},
				Timestamp: aihelpers.MatrixEventTimestamp(entry.Event),
			}
		}
		if meta, ok := userMessage.Metadata.(*MessageMetadata); ok && len(meta.CanonicalTurnData) == 0 {
			meta.CanonicalTurnData = aihelpers.TurnData{
				Role: "user",
				Parts: []aihelpers.TurnPart{{
					Type: "text",
					Text: meta.Body,
				}},
			}.ToMap()
