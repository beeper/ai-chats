...
func buildUserPromptTurn(blocks []PromptBlock) (PromptMessage, aihelpers.TurnData, bool) {
	msg := PromptMessage{Role: PromptRoleUser}
	td := aihelpers.TurnData{Role: "user"}
	msg.Blocks = make([]PromptBlock, 0, len(blocks))
	td.Parts = make([]aihelpers.TurnPart, 0, len(blocks))
	for _, block := range blocks {
		switch block.Type {
		case PromptBlockText:
			if strings.TrimSpace(block.Text) != "" {
				msg.Blocks = append(msg.Blocks, PromptBlock{Type: PromptBlockText, Text: block.Text})
				td.Parts = append(td.Parts, aihelpers.TurnPart{Type: "text", Text: block.Text})
			}
		case PromptBlockImage:
			...
			part := aihelpers.TurnPart{Type: "image", URL: block.ImageURL, MediaType: block.MimeType}
