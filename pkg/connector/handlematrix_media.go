...
func (oc *AIClient) handleMediaMessage(
	...
	if mediaURL == "" {
		return nil, aihelpers.UnsupportedMessageStatus(fmt.Errorf("%s message has no URL", msgType))
	}
	...
	if msgType == event.MsgFile {
		...
		case mimeType == "" || mimeType == "application/octet-stream":
			return nil, aihelpers.UnsupportedMessageStatus(errors.New("text file understanding is not supported"))
		}
	}

	if !ok {
		return nil, aihelpers.UnsupportedMessageStatus(fmt.Errorf("unsupported media type: %s", msgType))
	}
	...
	if msgType == event.MsgAudio || msgType == event.MsgVideo {
		...
		return nil, aihelpers.UnsupportedMessageStatus(fmt.Errorf(
	...
	if err != nil {
		return nil, aihelpers.MessageSendStatusError(err, "Couldn't prepare the media message. Try again.", "", messageStatusForError, messageStatusReasonForError)
	}

	userMeta := &MessageMetadata{
		BaseMessageMetadata: aihelpers.BaseMessageMetadata{
			Role: "user",
...
func (oc *AIClient) dispatchMediaUnderstandingFallback(
	...
	if err != nil {
		...
		return nil, aihelpers.MessageSendStatusError(err, userError, "", messageStatusForError, messageStatusReasonForError)
	}
	if description == "" {
		return nil, aihelpers.MessageSendStatusError(errors.New(emptyResult), userError, "", messageStatusForError, messageStatusReasonForError)
	}
